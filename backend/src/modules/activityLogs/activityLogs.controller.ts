...
export async function uploadAttachmentHandler(req: Request, res: Response) {
  ...
  const attachments = await service.addActivityAttachments(
    req.params.id,
    files.map((f) => ({ fileName: f.originalname, filePath: `uploads/activity-logs/${f.filename}`, fileSize: f.size, mimeType: f.mimetype })),
  );
